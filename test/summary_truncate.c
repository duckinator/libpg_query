...
void it_omits_CTE_definition(TestState* test_state) {
    TEST_INIT();
...
void it_omits_WHERE_clause(TestState* test_state) {
    TEST_INIT();
...
void it_omits_INSERT_field_list(TestState* test_state) {
    TEST_INIT();
...
void it_falls_back_to_simple_truncation(TestState* test_state) {
    TEST_INIT();
...
void it_handles_problematic_cases(TestState* test_state) {
    TEST_INIT();
...
void it_omits_UPDATE_target_list(TestState* test_state) {
    TEST_INIT();
...
void it_omits_ON_CONFLICT_target_list(TestState* test_state) {
    TEST_INIT();
...
void it_omits_ON_CONFLICT_target_list_2(TestState* test_state) {
    TEST_INIT();
    char* query = "\
...
void it_handles_GRANT(TestState* test_state) {
    TEST_INIT();
...
void it_does_not_segfault_on_target_list_from_CTE_already_removed_from_possible_truncations(TestState* test_state) {
    TEST_INIT();
    char* query = "\
        WITH activity AS ( \
...
void it_handles_CREATE_INDEX(TestState* test_state) {
    TEST_INIT();
...
void char_truncate_works(TestState* test_state) {
